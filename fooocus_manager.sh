...
install_app() {
    ...
    if git -C "$INSTALL_DIR" remote | grep -q '^origin$'; then
        $SUDO git -C "$INSTALL_DIR" remote set-url origin https://github.com/AsaTyr2018/Fooocus_revamp
    else
        $SUDO git -C "$INSTALL_DIR" remote add origin https://github.com/AsaTyr2018/Fooocus_revamp
    fi
}
